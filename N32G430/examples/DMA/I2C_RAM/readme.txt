1、功能说明
    该例程提供了一个DMA用法，用于在外围设备和RAM之间传输数据。
             
    初始化CLOCK，GPIO，PERIPH，然后为I2C启用DMA，再启用DMA。
    
    首先，DMA_CH6将I2C1_Tx_Buffer数据传输到I2C1的数据寄存器。
    然后I2C1发送数据到I2C2。
    最后，DMA_CH5将数据从I2C2的数据寄存器传输到I2C2_Rx_Buffer。
    
    等待DMA传输完成，比较I2C1_Tx_Buffer和I2C2_Rx_Buffer之间的数据一致性，并将比较结果输出到串口。
