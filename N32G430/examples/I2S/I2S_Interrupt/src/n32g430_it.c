...
extern uint16_t I2S_Master_Buffer_Tx[BufferSize], I2S_Master_Buffer_Rx[BufferSize], I2S_Slave_Buffer_Rx[BufferSize], I2S_Slave_Buffer_Tx[BufferSize];
extern uint32_t TxIdx1, RxIdx1, TxIdx2, RxIdx2;
...
/**
*\*\name    SPI1_IRQHandler.
*\*\fun     This function handles SPI1/I2S1 Handler.
*\*\param   none
*\*\return  none 
**/
void SPI1_IRQHandler(void)
{
    if(SPI_I2S_Interrupt_Flag_Status_Get(SPI1, SPI_I2S_INT_FLAG_TE) != RESET)
    {
        ...
        SPI_I2S_Data_Transmit(SPI1, I2S_Master_Buffer_Tx[TxIdx1++]);
        if(TxIdx1 == BufferSize)
        {
    ...
    {
        /* Store SPI_MASTER received data */
        I2S_Master_Buffer_Rx[RxIdx2++] = SPI_I2S_Data_Get(SPI1);
    }
}

/**
*\*\name    SPI2_IRQHandler.
*\*\fun     This function handles SPI2/I2S2 Handler.
*\*\param   none
*\*\return  none 
**/
void SPI2_IRQHandler(void)
{
    if(SPI_I2S_Interrupt_Flag_Status_Get(SPI2, SPI_I2S_INT_FLAG_TE) != RESET)
    {
        /* Send SPI_SLAVE data */
        SPI_I2S_Data_Transmit(SPI2, I2S_Slave_Buffer_Tx[TxIdx2++]);
        if(TxIdx2 == BufferSize)
        {
            /* Disable SPI_SLAVE TXE interrupt */
            SPI_I2S_Interrupts_Disable(SPI2, SPI_I2S_INT_TE);
        }
    }
    if(SPI_I2S_Interrupt_Flag_Status_Get(SPI2, SPI_I2S_INT_FLAG_RNE) == SET)
    {
        /* Store SPI_SLAVE received data */
        I2S_Slave_Buffer_Rx[RxIdx1++] = SPI_I2S_Data_Get(SPI2);
    }
}
