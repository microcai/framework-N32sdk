...
int main(void)
{
    ...
    DMA_Channel_Enable(USARTz_Rx_DMA_Channel);
    ...
    while (DMA_Flag_Status_Get(DMA,USARTy_Rx_DMA_FLAG) == RESET)
    {
    }
    ...
    while (DMA_Flag_Status_Get(DMA, USARTz_Rx_DMA_FLAG) == RESET)
    {
    }
...
void DMA_Configuration(void)
{
    ...
    DMA_Channel_Request_Remap(USARTz_Tx_DMA_Channel, USARTz_Tx_DMA_REMAP);

    /* USARTz RX DMA1 Channel (triggered by USARTz Rx event) Config */
    DMA_Reset(USARTz_Rx_DMA_Channel);
    DMA_InitStructure.PeriphAddr = USARTz_DAT_Base;
    DMA_InitStructure.MemAddr    = (uint32_t)RxBuffer2;
    DMA_InitStructure.Direction  = DMA_DIR_PERIPH_SRC;
    DMA_InitStructure.BufSize    = TxBufferSize1;
    DMA_Initializes(USARTz_Rx_DMA_Channel, &DMA_InitStructure);
    DMA_Channel_Request_Remap(USARTz_Rx_DMA_Channel, USARTz_Rx_DMA_REMAP);
}
