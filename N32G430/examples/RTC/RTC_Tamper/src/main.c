...
void Tamper_Input_Io_Init(void);
...
{    
    ...
    if(SUCCESS == RTC_CLKSource_Config(RTC_CLK_LSE, RTC_CLK_FIRST_CONFIG))
    {
        if (USER_WRITE_BKP_DAT1_DATA != RTC_Backup_Register_Read(RTC_BACKUP_REGISTER_1) )
        {
            ...
            Tamper_Input_Io_Init();
            /* Configure Tamper GPIO trigger mode */
            RTC_Tamper_Trigger_Config(RTC_TAMPER_2, RTC_TamperTrigger_RisingEdge);
            /* Configure the valib number of scanning times */
            RTC_Tamper_Filter_Config(RTC_TamperFilter_Disable);
            ...
            RTC_Tamper_Pins_Precharge_Duration(RTC_TamperPrechargeDuration_1RTCCLK);
    ...
    RTC_Tamper_Interrput_Enable(RTC_TAMPER2_INT);
    /* Enable tamper2 */
    RTC_Tamper_Enable(RTC_TAMPER_2);
    while (1)
    {
    }
}
...
/**
*\*\name    Tamper_Input_Io_Init
*\*\fun     tamper input pin init
*\*\param   none
*\*\return  none
**/
void Tamper_Input_Io_Init(void)
{
    ...
    /* Configure tamper pin in alternate function mode */
...
/**
*\*\name    TAMPER_INT_Configuration
*\*\fun     Configure the tamper interruput,EXTI18.
*\*\param   cmd
                - ENABLE
                - DISABLE
*\*\return  none
